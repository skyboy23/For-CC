'''
/root/autodl-tmp/code/43.algonauts23-CC/config/phase2_finetune_unireplk_2_1218.yaml
--out_dir
/root/autodl-tmp/code/43.algonauts23-CC/scripts/results
--ckpt
/root/autodl-tmp/code/43.algonauts23-CC/scripts/results/algonauts23-group-encoder/250331171553-huge-boy/checkpoints/ckpt-best.pt
--workers
1
...
def make_collate(transform: Callable):
    def collate_fn(batch: List[Dict[str, torch.Tensor]]):
        ...
        collated = {k: torch.stack(v) if all(isinstance(item, torch.Tensor) for item in v) else v for k, v in
                    collated.items()}
...
def validate(
    ...
    if args.figures:
        ...
        # 保存到本地文件
        np.save(fig_dir / 'group_target_scores_OURS.npy', group_target_scores)
        # 保存列表到本地
        with open(fig_dir / 'target_scores_OURS.pkl', 'wb') as f:
            pickle.dump(list(target_scores.values()), f)

        # 保存列表到本地
        with open(fig_dir / 'map_titles_OURS.pkl', 'wb') as f:
            pickle.dump(map_titles, f)
