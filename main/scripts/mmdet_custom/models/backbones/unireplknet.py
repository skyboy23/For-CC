...
class UniRepLKNetBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.pwconv1 = nn.Sequential(
            ...
            # lora.Linear(dim, ffn_dim, r=R_lora))
        ...
        if deploy:
            self.pwconv2 = nn.Sequential(
                ...
                # lora.Linear(ffn_dim, dim, r=R_lora),
                NHWCtoNCHW())
        else:
            self.pwconv2 = nn.Sequential(
                nn.Linear(ffn_dim, dim, bias=False),
                # lora.Linear(ffn_dim, dim, bias=False, r=R_lora),
                NHWCtoNCHW(),
        ...
        for i in range(4):
            main_stage = nn.Sequential(
                *[UniRepLKNetBlock(dim=dims[i], kernel_size=kernel_sizes[i][j], drop_path=dp_rates[cur + j],
                                   ...
                                   attempt_use_lk_impl=attempt_use_lk_impl,
                                   with_cp=with_cp, use_sync_bn=use_sync_bn, R_lora=1) for j in
                  range(depths[i])])
    ...
    class UniRepLKNetBackbone(UniRepLKNet):
        def __init__(self,
            ...
            super().__init__(in_chans=3, num_classes=None, depths=depths, dims=dims,
                             ...
                             init_cfg=init_cfg, attempt_use_lk_impl=attempt_use_lk_impl, use_sync_bn=False)
