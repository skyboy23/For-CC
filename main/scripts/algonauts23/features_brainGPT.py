...
class FeatureExtractor:
    ...
    def __call__(self, *args: Any, **kwargs: Any) -> Tuple[Any, Dict[str, Tensor]]:
        ...
        output_select_features = self._features.copy()

        return output, output_select_features
    ...
    def expand_layers(model: nn.Module, layers: List[str]) -> List[str]:
        ...
        patterns in `layers`.
        """
        all_layers = [name for name, _ in model.named_modules() if len(name) > 0]  # 拿到所有的layer层的名称
