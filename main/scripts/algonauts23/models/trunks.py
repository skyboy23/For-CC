...
def create_trunk(name: str, **kwargs) -> Tuple[torch.nn.Module, DataConfig]:
    ...
    # Note, this is the only case where kwargs are passed through
        ...
        # model = create_model(name, pretrained=True, **kwargs)  # 直接下载，因为网络原因，失败

        pretrained_cfg = create_model(name, **kwargs).default_cfg
        # pretrained_cfg['file'] = '/home/mashuxiao/code/43.algonauts23-main_BlobGPT/eva02clip/model.safetensors'
        pretrained_cfg['file'] = '/home/yangjie/mashuxiao/code/43.algonauts23-main_BlobGPT/eva02clip/model.safetensors'
        model = create_model(name, pretrained=True, pretrained_cfg=pretrained_cfg, **kwargs)
