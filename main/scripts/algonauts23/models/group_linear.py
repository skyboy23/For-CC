...
from scripts.algonauts23 import FMRI_DIM, NUM_SUBS
...
class GroupLinearEncoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # 考虑增加投射层，完成从LLM到映射前的转换

        # Shared and subject-specific projection
        self.shared_proj = nn.Linear(hidden_dim, proj_dim, bias=False)  # hidden_dim=1024, proj_dim=2048
        self.subj_proj = GroupLinear(hidden_dim, proj_dim, group_size, bias=True)  # hidden_dim=1024, proj_dim=2048 group_size=8
        self.embed = nn.Linear(proj_dim, out_dim, bias=True)

    def forward(
        self, features: List[torch.Tensor], indices: Optional[torch.Tensor]
    ) -> torch.Tensor:
        # print(len(features))       # 正常模式下 此值 是 6
        # print(features[0].shape)   # 正常模式下 128, 257, 768
        pred, _ = self.predict(features, indices)  # features：6层，每层512*257*768
        return pred
    ...
    def predict(
        ...
        group_pred = self.embed(self.shared_proj(latent))  # self.shared_proj(latent)是512*2048，group_pred是512*39548
        if indices is None:
            return group_pred, group_pred

        # self.subj_proj作用：原来是8个被试，用这个函数根据indices中标记的image的subjID进行选择，把512个image中的对应被试的数据找出来
...
class FactorLinear2D(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        # x: (N, L, C)
        assert x.ndim == 3 and x.shape[1:] == self.in_shape  #  我这里的in_shape是个list；原始的变量 此处是 tuple
...
class GroupLinear(nn.Module):
    ...
    def __init__(
        ...
        self.fc = nn.Linear(in_features, group_size * out_features, bias=bias)  # in 1024 out 8*2048=16384
    ...
    def forward(self, x: torch.Tensor, indices: torch.Tensor) -> torch.Tensor:
        ...
        x = torch.take_along_dim(x, indices, 1)  # 根据维度1和下标indices选值
        x = x.squeeze(1)
        return x
