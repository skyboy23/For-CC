...
class MiniGPT5_Model(LightningModule):
    def __init__(self,
        ...
        sd_model_name = "../weights/WEIGHT_FOLDER/stable-diffusion-2-1-base"
        ...
        if len(ALL_FMRI_TOKENS):
            self.output_fmri_id = self.tokenizer.convert_tokens_to_ids(ALL_FMRI_TOKENS[0])
            # self.test = self.tokenizer.convert_tokens_to_ids("[SEP]")
        self.fmri_token_num = FMRI_TOKEN_NUM
        ...
        zero_fmri_feature = torch.zeros((1, self.fmri_token_num, hidden_size), dtype=TRAINABLE_PRECISION)
        self.register_buffer('zero_fmri_feature', zero_fmri_feature, persistent=False)
    ...
    def forward(self, input_ids, attention_mask, input_images, output_image, labels, captions=None,
                input_images_feature=None, output_image_feature=None):
            ...
            if USE_CFG and random.random() < 0.1:
                ...
                if calculate_caption_loss:
                    caption_feature.append(
                        self.encode_caption(captions[bs_id], self.sd_tokenizer.model_max_length, inference=True))
        ...
        if len(t2i_input_embedding) == 0:
            ...
            # else:
            #     image_loss = self.compute_image_loss(mapping_feature, None, output_image_feature=output_image_feature[
    ...
    #             input_images_feature=None, output_image_feature=None):
    #     if self.encoder_model_config.model_type == 'multimodal_encoder':
    #         outputs, special_token_index, output_fmri_token_index = self.model(input_ids=input_ids, labels=labels,
    ...
    #         # random set 10% data with empty text feature
    #         if USE_CFG and random.random() < 0.1:
    #             t2i_input_embedding.append(self.zero_fmri_feature)
    #             # if calculate_caption_loss:
    #             #     caption_feature.append(self.empty_text_feature)
    #         else:
    #             t2i_input_embedding.append(last_hidden_state[bs_id:bs_id + 1, seq_id:seq_id + self.img_token_num, :])
    ...
    #             #         self.encode_caption(captions[bs_id], self.sd_tokenizer.model_max_length, inference=True))
    #
    #     assert len(t2i_input_embedding) > 0, "len(t2i_input_embedding) > 0"
    #
    #     t2i_input_embedding = torch.cat(t2i_input_embedding, dim=0)
    #     img_token_bs = t2i_input_embedding.shape[0]
    #     t2i_input_embedding = self.fc(t2i_input_embedding)
    ...
    #                                               tgt=self.t2i_decoder_prompt.repeat(img_token_bs, 1, 1))
    #
    #     # 0722版本新加，计算LLM输出文本特征和Loss的计算
    #     t2f_input_embedding = []
    #     for i in range(len(output_fmri_token_index)):
    #         bs_id, seq_id = output_fmri_token_index[i]
    #         # random set 10% data with empty text feature
    #         if USE_CFG and random.random() < 0.1:
    #             t2f_input_embedding.append(self.zero_img_feature)
    #             # if calculate_caption_loss:
    #             #     caption_feature.append(self.empty_text_feature)
    #         else:
    #             t2f_input_embedding.append(last_hidden_state[bs_id:bs_id + 1, seq_id:seq_id + self.img_token_num, :])
    #             # if calculate_caption_loss:
    #             #     caption_feature.append(
    #             #         self.encode_caption(captions[bs_id], self.sd_tokenizer.model_max_length, inference=True))
    #
    #     assert len(t2f_input_embedding) > 0, "len(t2f_input_embedding) > 0"
    #
    #     # t2i_input_embedding = torch.cat(t2i_input_embedding, dim=0)
    #     # img_token_bs = t2i_input_embedding.shape[0]
    #     # t2i_input_embedding = self.fc(t2i_input_embedding)
    #     # fmri_feature = self.llm_to_t2i_mapping(src=t2i_input_embedding,
    #     #                                           tgt=self.t2i_decoder_prompt.repeat(img_token_bs, 1, 1))
    #
    #     return mapping_feature, t2f_input_embedding

    def generate(self, utterance, input_image=None, task_name=None, max_new_tokens=256, force_generation=False,
        ...
        llm_sample_outputs = self.model.predict(utterance, input_image, max_new_tokens=max_new_tokens, temperature=1.0,
                                                repetition_penalty=2.0, task_name=task_name,
        ...
        if len(special_token_index):
            ...
            t2i_input_embedding = self.fc(t2i_input_embedding)
            mapping_feature = self.llm_to_t2i_mapping(src=t2i_input_embedding, tgt=self.t2i_decoder_prompt)  # 1 77 1024
            ...
            #     predicted_images_ft = self.image_pipeline(prompt_embeds=mapping_feature,
            #                                               negative_prompt_embeds=empty_feature,
            #                                               guidance_scale=guidance_scale).images[0]
            # else:
            #     predicted_images_ft = self.image_pipeline(prompt_embeds=mapping_feature, guidance_scale=guidance_scale,
            #                                               use_original=True).images[0]
            # print("eee")
