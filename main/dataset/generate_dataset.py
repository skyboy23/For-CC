...
from scripts.algonauts23 import SUBS

ROOT = Path(__file__).parent
SEED = 2023
PARENT_SPLITS = {"train": "training", "val": "training",
                 "testval": "training", "test": "test"}
...
def generate_dataset(root: Path, split: str = "train"):
    ...
    for sub_id, sub in enumerate(SUBS):
        ...
        if parent_split != "test":
            ...
            pca = PCA(n_components=2048, svd_solver="randomized")
            # temp_data = pca.fit(fmri)
            fmri_pca = pca.fit_transform(fmri)
            logging.info("fmri_pca shape: %s", fmri_pca.shape)
        else:
            fmri = None
            fmri_pca = None
        ...
        for idx in tqdm(indices):
            ...
            activity_pca = fmri_pca[idx] if fmri is not None else None
...
if __name__ == "__main__":
    ...
    dset = Dataset.from_generator(generate_dataset, gen_kwargs={"root": ROOT, "split": args.split})
